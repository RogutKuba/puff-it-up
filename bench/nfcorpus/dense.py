"""
Dense vector search benchmark on NFCorpus dataset using turbopuffer and OpenAI embeddings.
...
NAMESPACE = "nfcorpus-dense-bench-openai"
...
def get_next_result_filename() -> str:
    """Get the next available result filename (nfcorpus_dense_N.txt)."""
    os.makedirs(RESULTS_DIR, exist_ok=True)
    existing = os.listdir(RESULTS_DIR)
    max_num = 0
    for fname in existing:
        match = re.match(r"nfcorpus_dense_(\d+)\.txt", fname)
        if match:
            max_num = max(max_num, int(match.group(1)))
    return os.path.join(RESULTS_DIR, f"nfcorpus_dense_{max_num + 1}.txt")
...
def main():
    ...
    print("=" * 60)
    print("Dense Vector Search Benchmark - NFCorpus Dataset")
    print("=" * 60)
