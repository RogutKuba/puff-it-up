"""
Late interaction (ColBERT-style) benchmark on ArguAna dataset.
Uses Jina ColBERT v2 for multi-vector embeddings and turbopuffer for storage/search.
Implements two-stage retrieval with MaxSim scoring.
"""
...
FIRST_STAGE_TOP_K = 50  # Per-token candidates in first stage (higher for better recall)
...
def get_next_result_filename() -> str:
    """Get the next available result filename (arguana_late_N.txt)."""
    os.makedirs(RESULTS_DIR, exist_ok=True)
    existing = os.listdir(RESULTS_DIR)
    max_num = 0
    for fname in existing:
        match = re.match(r"arguana_late_(\d+)\.txt", fname)
        if match:
            max_num = max(max_num, int(match.group(1)))
    return os.path.join(RESULTS_DIR, f"arguana_late_{max_num + 1}.txt")
...
def main():
    ...
    print("=" * 60)
    print("Two-Stage Late Interaction Benchmark - ArguAna Dataset")
    print("=" * 60)
