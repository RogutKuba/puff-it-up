"""
Dense vector search benchmark on SciFact dataset using turbopuffer and Qwen3 embeddings.
...
def get_next_result_filename() -> str:
    """Get the next available result filename (dense_N.txt)."""
    os.makedirs(RESULTS_DIR, exist_ok=True)
    existing = os.listdir(RESULTS_DIR)
    max_num = 0
    for fname in existing:
        match = re.match(r"dense_(\d+)\.txt", fname)
        if match:
            max_num = max(max_num, int(match.group(1)))
    return os.path.join(RESULTS_DIR, f"dense_{max_num + 1}.txt")
...
def search_queries(
    ...
    for i in tqdm(range(0, len(query_ids), BATCH_SIZE), desc="Searching"):
        ...
        for qid, embedding in zip(batch_ids, embeddings):
            ...
            # # DEBUG: Save first 10 queries with top 3 results
            # if query_count < 10:
            #     debug_results.append({
            #         "query_id": qid,
            #         "query_text": queries[qid],
            #         "top_3_results": results[qid][:3],
            #     })
            #     query_count += 1

            #     if query_count == 10:
            #         output_path = os.path.join(os.path.dirname(__file__), "dense_debug_results.json")
...
def main():
    ...
    print("=" * 60)
    print("Dense Vector Search Benchmark - SciFact Dataset")
    print("=" * 60)
