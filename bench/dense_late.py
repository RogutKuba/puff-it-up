...
Uses Qwen3 8B for candidate retrieval, Jina ColBERT v2 for MaxSim reranking.
...
DENSE_NAMESPACE = "scifact-dense-bench-openai"
NAMESPACE = "scifact-dense-bench-openai"
LATE_NAMESPACE = "scifact-late-bench"
...
def embed_dense(client: OpenAI, texts: list[str]) -> list[list[float]]:
    """Get dense embeddings from Qwen3 8B."""
...
def search_queries(
    ...
    # DEBUG: Only process first 10 queries and save results
    debug_results = []
    query_count = 0
    ...
    for qid, query_text in tqdm(queries.items(), desc="Searching"):
        ...
        # Get first-stage candidates for debug
        first_stage_candidates = first_stage_dense(dense_ns, dense_embedding)
        ...
        # DEBUG: Save first 10 queries with top 3 results from both stages
        if query_count < 10:
            debug_results.append({
                "query_id": qid,
                "query_text": query_text,
                "first_stage_top_3": first_stage_candidates[:3],
                "final_top_3": doc_ids[:3],
            })
            query_count += 1

            if query_count == 10:
                output_path = os.path.join(os.path.dirname(__file__), "dense_late_debug_results.json")
                with open(output_path, "w") as f:
                    json.dump(debug_results, f, indent=2)
                print(f"\nDEBUG: Saved first 10 queries to {output_path}")
...
def main():
    ...
    print(f"\nRetrieval parameters:")
    print(f"  First stage (dense) top-k: {FIRST_STAGE_TOP_K}")
